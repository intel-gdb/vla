  ...
  if (nss != ndimensions)
  ...
  else
    {
      for (i = lowerbound; i < upperbound + 1 && (*elts) < options->print_max;
	   i++, (*elts)++)
	{
	  struct value *subarray = value_subscript ((struct value *)val, i);

	  val_print (value_type (subarray),
		     value_contents_for_printing (subarray),
		     value_embedded_offset (subarray),
		     value_address (subarray), stream, recurse,
		     subarray, options, current_language);
